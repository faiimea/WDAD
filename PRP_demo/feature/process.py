...
def fast_glcm_ASM(img, vmin=0, vmax=255, nbit=8, ks=5):
    ...
    for i in range(nbit):
        for j in range(nbit):
            asm  += glcm[i,j]**2
    ene = np.sqrt(asm)
    # asm=Image.fromarray(asm)
    # trans_resize=transforms.Resize([256,256])
    # asm=trans_resize(asm)
    return asm, ene

# def ToBGR(save_path,img_path="D:/PRP_lfz/PRP/ORG_img"):
#     #列出指定路径下的所有文件
#     imgPdir=os.listdir(img_path)
#     imgdir=img_path+"/"+imgPdir[1]
#     # print(imgPdir)
#     s=imgdir
...
#     #asm即为所需
#     img_file="D:/PRP_lfz/PRP/GRAY_img"
#     if not os.path.exists(img_file):
# 	    os.mkdir(img_file)
#     cv2.imwrite(img_file,asm)
    # plt.imshow(tr)
    # plt.show()
...
img_path="D:/PRP_lfz/PRP/FGSM_img"
imgPdir=os.listdir(img_path)
